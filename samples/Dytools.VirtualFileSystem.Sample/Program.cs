// Dytools.VirtualFileSystem.Sample - demonstrates core VFS features:
...
Console.WriteLine("READY - attach/start profiler, then press Enter");
...
static void Pass(string msg) => Console.WriteLine($"  ✓ {msg}");

static async Task WriteText(IVirtualFileSystem vfs, string path, string text)
{
    await using var stream = await vfs.OpenWriteAsync(path);
    await stream.WriteAsync(Encoding.UTF8.GetBytes(text));
}

static async Task<string> ReadText(IVirtualFileSystem vfs, string path)
{
...
sealed class SymlinkAwareInMemoryNode : VfsNodeBase, ISymlinkCapableNode
{
...
sealed class NullNode : VfsNodeBase {
    ...
    public override IAsyncEnumerable<VfsNodeInfo> ListAsync(VfsNodeRequest request, CancellationToken ct = default) {
        throw new NotImplementedException();
    }
}

sealed class DeduplicatingInMemoryNode : Dytools.VirtualFileSystem.VfsNodeBase, IDeduplicatingNode
{
...
sealed class SimpleHardLinkStore(
...
sealed class CommitStream(Action<byte[]> commit) : MemoryStream
{
